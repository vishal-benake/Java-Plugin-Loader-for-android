...
import com.skullshooter.ssplugin.app.activities.OAuthVreifications;
...
import static com.skullshooter.ssplugin.app.activities.OAuthVreifications.aboutLogin;
import static com.skullshooter.ssplugin.app.activities.OAuthVreifications.layoutInfo;
import static com.skullshooter.ssplugin.app.activities.OAuthVreifications.logFrag;
import static com.skullshooter.ssplugin.app.activities.OAuthVreifications.login_button;

public class Authantications extends AsyncTask<String, Void, String> {
    private final WeakReference<OAuthVreifications> weakActivity;
    private final ProgressDialog pDialog;
    private static String URLSERVER = "https://BACKEND_URL_FOR_LOGIN/ex-login.pho";
    Timer _t;
    int _count;
    ...
    public Authantications(OAuthVreifications activity){
    ...
    protected void onPreExecute() {
        OAuthVreifications activity = getActivity();
        if (activity == null) {
            return;
        }
        ...
        if (getDialog() != null) {
            ...
            try {
                ...
                aboutLogin.setText(R.string.authen);
    ...
    protected void onPostExecute(String s) {
        ...
        if(s == null || s.isEmpty()){
            Toast.makeText(activity, "Auth error: code 1. invalid account credential", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        try {
            ...
            if (data.get("Status").toString().equals("Success")) {
                Preferences prefs = Preferences.with(activity);
                String EXPIRED = "EXPIRED";
                prefs.write(EXPIRED, data.get("SubscriptionLeft").toString());
                String loginSuccess = "loginSuccess";
                String isSuccess = "statusSuccess";
                prefs.write(loginSuccess, isSuccess);
                loginLoaded = true;
                getActivity().startActivity(new Intent(activity, a.c.class));
                getActivity().finish();
            } else {
    ...
    private OAuthVreifications getActivity() {
